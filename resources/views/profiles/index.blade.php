...
<body class="bg-gray-50 dark:bg-[#393053]">
  ...
  <div class="sm:ml-64 mt-16 px-0">
    ...
    <div class="pg-identity container">
          ...
          @if (!empty($profile->kontak) && is_array($profile->kontak))
            <div class="pg-social-list">
              @if (!empty($profile->kontak['facebook']))
                <a href="{{ $profile->kontak['facebook'] }}"
              ...
              @if (!empty($profile->kontak['instagram']))
                <a href="{{ $profile->kontak['instagram'] }}"
                   target="_blank" rel="noopener"
      ...
      {{-- KANAN: bio / panel upgrade --}}
      <div class="pg-right">
        @if($user->role === 'seniman')
          <div class="pg-bio" role="region" aria-label="Deskripsi">
            {{ $profile->bio ?: 'Deskripsi singkat user. Bisa beberapa kalimat untuk memperkenalkan diri dan karya.' }}
          </div>
        @else
          <div class="pg-upgrade-panel">
            ...
            <p class="text-gray-200 text-sm mb-2 leading-relaxed">
              Upgrade akun Anda menjadi seniman untuk menambahkan bio, sosial media, dan memamerkan karya Anda.
            </p>
            <a href="{{ route('upgrade.akun') }}" class="pg-btn-upgrade">
              Upgrade ke Seniman
            </a>
          </div>
        @endif
